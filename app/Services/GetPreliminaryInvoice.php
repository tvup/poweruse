...
class GetPreliminaryInvoice
{
            ...
            foreach ($tariffs as $tariff) {
                ...
                if ($datahubPriceList) {
                    ...
                    if (count($netPrices) > 1) {
                        ...
                        if (!isset($debugLoggedDpl[$tariffName]) || $debugLoggedDpl[$tariffName] < 8) {
    ...
    public function getCostOfCustomUsage(array $meterData, string $refreshToken, string $price_area, float|string $overhead = 0.048, User $user = null): array
    {
        ...
        try {
            ...
            $charges = cache($key);
            if (!$charges) {
                $charges = $this->meteringDataService->getCharges($start_date, $end_date, null, ['refresh_token' => $refreshToken], $user);
                $expiresAt = Carbon::now()->addMonthsNoOverflow(1)->startOfMonth();
                cache([$key => $charges], $expiresAt);
            }
            list($subscriptions, $tariffs) = $charges;
